...
**Граф вариаций прохода по вкладкам («паутина», как в Obsidian)** — интерактивный [HTML](diagrams/nav-graph.html) (offline: перетаскивание узлов, зум колесом, панорама, подсветка соседей) · [SVG](diagrams/nav-graph.svg) · [PNG](diagrams/nav-graph.png)

> 45 узлов (страницы + вкладки) и 68 переходов. Цвет узла — зона/роль, размер — число связей. Скачайте `nav-graph.html` и откройте в браузере — граф живой, как в Obsidian.

![Граф навигации ForkWork](diagrams/nav-graph.png)
