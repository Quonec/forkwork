      ...
      <div className="mt-6 flex gap-1 overflow-x-auto border-b border-stone-200">
        {TABS.filter(([k]) => k !== "become" || user.role === "customer").map(([key, label]) => (
          <Link
            ...
            className={`whitespace-nowrap border-b-2 px-4 py-2.5 text-sm font-semibold ${tab === key ? "border-orange-500 text-orange-600" : "border-transparent text-stone-500 hover:text-stone-800"}`}
          >
            {label}
          </Link>
        ))}
